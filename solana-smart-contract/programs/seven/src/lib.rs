...
pub mod luckybastard {
    ...
    pub fn setup(ctx: Context<Setup>, vendor: Pubkey, bet_amount: u64, bet_amount_jack: u64, player_choice: u64, player_choice_jack: u64, player_seed: i64) -> Result<()> {
        ...
        invoke(
            &transfer(
                ...
                seventy.bet_amount*1000/1000, //(total_bet/10) - seventy.bet_amount,
            ),
            &[
        ...
        if seventy.player_choice_jack != 0 {
            msg!("transfer jackpot to game wallet");
            invoke(
                &transfer(
                    ctx.accounts.player.to_account_info().key,
                    seventy.to_account_info().key,
                    seventy.bet_amount_jack*1000/1000, //(total_bet/10) - seventy.bet_amount,
                ),
                &[
                    ctx.accounts.player.to_account_info(),
                    seventy.to_account_info(),
    ...
    pub fn play(ctx: Context<Play>, vendor_seed: i64) -> Result<()> {
        ...
        } else if seventy.player_choice_jack < 3 && seventy.player_choice > 2 { // JackPot2
            seventy.bet_amount_jack * 300
        } else if seventy.player_choice_jack < 3 && seventy.player_choice < 3 { // JackPot2
            seventy.bet_amount_jack * 360
        } else if seventy.player_choice_jack > 2 && seventy.player_choice > 2 { //JackPot3
            seventy.bet_amount_jack * 150
        } else if seventy.player_choice_jack > 2 && seventy.player_choice < 3{ //JackPot3
            seventy.bet_amount_jack * 180
        } else {  //JackPot11
            seventy.bet_amount_jack * 180
        };
        ...
        /*msg!("Send Tax to wallet!");
        **seventy.to_account_info().try_borrow_mut_lamports()? -= seventy.bet_amount*35/1000;
        **ctx.accounts.vault.to_account_info().try_borrow_mut_lamports()? += seventy.bet_amount*35/1000;
        if seventy.player_choice_jack != 0 {
            msg!("Send JackPot Tax to wallet!");
            **seventy.to_account_info().try_borrow_mut_lamports()? -= seventy.bet_amount_jack*35/1000;
            **ctx.accounts.vault.to_account_info().try_borrow_mut_lamports()? += seventy.bet_amount_jack*35/1000;
        }*/
...
pub struct Setup<'info> {
    #[account(
        ...
        space = Seventy::LEN,
        seeds = [b"luckybastard", player.key().as_ref(), vendor.as_ref()], bump //seeds = [b"seventy", vendor.key().as_ref(), player.as_ref()], bump
    )]
...
pub struct Play<'info> {
    #[account(
        mut,
        seeds = [b"luckybastard", player.key().as_ref(), vendor.key().as_ref()], bump //  seeds = [b"seventy", vendor.key().as_ref(), player.key().as_ref()], bump
    )]
    pub seventy: Account<'info, Seventy>,
    #[account(mut)]
...
pub struct Delete<'info> {
    #[account(
        ...
        seeds = [b"luckybastard", player.key().as_ref(), vendor.key().as_ref()], bump         //seeds = [b"seventy", vendor.key().as_ref(), player.as_ref()], bump
    )]
    pub seventy: Account<'info, Seventy>,
    #[account(mut)]
